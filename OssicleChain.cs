...
public class OssicleChain : MonoBehaviour
{
    ...
    void Update()
    {
        if (enableRealtimePhysics && Application.isPlaying)
        {
    ...
    void UpdateOssiclePhysics()
    {
        ...
        if (malleusRb != null)
        {
            malleusData.currentAngle = malleus.transform.eulerAngles.y;
            malleusData.angularVelocity = malleusRb.angularVelocity.y;
            malleusData.currentPosition = malleus.transform.position;
        }
        
        if (incusRb != null)
        {
            incusData.currentAngle = incus.transform.eulerAngles.y;
            incusData.angularVelocity = incusRb.angularVelocity.y;
            incusData.currentPosition = incus.transform.position;
        }
        
        if (stapesRb != null)
        {
            stapesData.currentAngle = stapes.transform.eulerAngles.z;
            stapesData.angularVelocity = stapesRb.angularVelocity.z;
            stapesData.currentPosition = stapes.transform.position;
        }
    ...
    void CalculateOutputVibration()
    {
        ...
        // 각 단계별 전달 계산
        float malleusOutput = inputVibration * GetFrequencyResponse(currentFrequency);
    ...
    float GetFrequencyResponse(float frequency)
    {
        ...
        if (frequency < 100f)
            ...
            return Mathf.Lerp(1.0f, 0.5f, (frequency - 4000f) / 4000f);
        else
            return 0.5f;
    }
    ...
    float CalculateFrequencyAttenuation(float frequency)
    {
        ...
        float q = 2.0f; // Q factor
        
        float omega = 2 * Mathf.PI * frequency;
        float omega0 = 2 * Mathf.PI * resonanceFreq;
        
        // 2차 시스템 응답
        float magnitude = 1.0f / Mathf.Sqrt(
            Mathf.Pow(1 - Mathf.Pow(omega / omega0, 2), 2) + 
            Mathf.Pow((omega / omega0) / q, 2)
        );
